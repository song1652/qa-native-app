"""
tc_ios_settings_v1.py — Ios | iOS Settings 기본 검증
자동 생성: Capture Studio (2026-09-12 15:30)
"""
...
class TestTcIosSettingsV1:
    """Capture Studio — iOS Settings 기본 검증"""
    ...
    def test_tc_ios_settings_v1(self):
        """단계별 동작 및 검증"""
        # Step 1: '일반' 탭
        self._el('AppiumBy.ACCESSIBILITY_ID', '일반').click()
        ...
        if not _found_text:
            ...
            try:
                _fb_el = self.driver.find_element(
                    AppiumBy.XPATH, "//*[@text='일반']")
                _found_text = _fb_el is not None
            except NoSuchElementException:
                pass
        assert _found_text, f"페이지 소스·XPath에서 '일반' 없음"
        # Step 3: 이전 화면 복귀
        self.driver.back()
        _time.sleep(1.0)  # 화면 복귀 대기
        # Step 4: 메인 복귀 확인 — '일반' 노출
        _found_text = False
        for _retry_i in range(5):
            _src = self.driver.page_source
            if '일반' in _src:
